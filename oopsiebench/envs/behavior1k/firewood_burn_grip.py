"""
Task configuration for **firewood_burn_grip**.

Scene : Rs_int
Robot : FrankaPanda (franka0)
Damage: thermal (robot gripper self-burn)

Single-task refinement of add_firewood.py: identical scene/robot/camera/objects,
but the fireplace's ``heatSource`` is widened and heated (distance_threshold
0.12->0.20, temperature 100.0->160.0, heating_rate 0.1->0.15) so the gripper is
more likely to take thermal damage while reaching for/placing logs near the
flame. Object count is unchanged from add_firewood, so the existing
``add_firewood.pkl`` init state is safely reused.
"""
...
TASK_OBJECTS = {
    "fireplace": {
        ...
        "abilities": {
            "heatSource": {
                # Wider + hotter than add_firewood's tamed values, deliberately
                # so the gripper wanders into the heat zone while reaching for logs.
                "temperature": 160.0,
                "heating_rate": 0.15,
        ...
        "initial_state": {"temperature": 160.0},
    },
    "log_center": {
        "type": "DatasetObject",
        "name": "log_center",
...
def get_task_config() -> TaskConfig:
    return TaskConfig(
        ...
        default_collect_hdf5="demos/behavior1k/teleop_data/firewood_burn_grip.hdf5",
        default_playback_hdf5="demos/behavior1k/playback_data/firewood_burn_grip_playback.hdf5",
        default_video_dir="demos/behavior1k/playback_videos/firewood_burn_grip",
    )
