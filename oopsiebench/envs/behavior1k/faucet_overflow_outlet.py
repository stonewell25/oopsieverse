"""
Task configuration for **faucet_overflow_outlet**.

Scene : house_single_floor (kitchen_0, sink cluster — same spot as fill_bowl
        / turn_on_faucet)
Robot : FrankaMounted (franka0)
Damage: mechanical + electrical (bowl + laptop + robot)

Recombination of fill_bowl.py: same bowl/place_mat/robot spawn/camera/pkl,
plus a bystander ``laptop`` (same category/model as pour_water.py's laptop —
the only category in this codebase with a tuned electrical ``PARAMS`` block
near water) placed on the counter edge, standing in for an electrical hazard
near the sink since no verified ``electric_switch``/outlet asset placement
...
def get_task_config() -> TaskConfig:
    return TaskConfig(
        task_name="faucet_overflow_outlet",
        ...
        target_objects_health=[ROBOT_NAME, "bowl", "laptop"],
        ...
        default_collect_hdf5="demos/behavior1k/teleop_data/faucet_overflow_outlet.hdf5",
        default_playback_hdf5="demos/behavior1k/playback_data/faucet_overflow_outlet_playback.hdf5",
        default_video_dir="demos/behavior1k/playback_videos/faucet_overflow_outlet",
    )
...
def reset(env):
    ...
    # Laptop isn't part of the saved pkl state; it already has the correct
    # static pose from TASK_OBJECTS, just make sure it's settled.
