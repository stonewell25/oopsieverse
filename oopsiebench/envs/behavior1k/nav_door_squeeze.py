"""
Task configuration for **nav_door_squeeze**.

Scene : Rs_int (Tiago primitives-style)
Robot : Tiago (tiago0)
Damage: mechanical

Recombination of nav_to_table.py: 100% identical TASK_OBJECTS/robot/camera/
pkl; the only change is appending ``"door"`` to ``load_object_categories``
so Rs_int's own native door (if present in the loaded rooms) loads at its
correct dataset transform — zero new coordinates, since we don't know its
exact instance name/pose offline. Tracked by *category* (not name) in
damage_params.py's DAMAGEABLE_OBJECTS so whatever native door object loads
is automatically damage-tracked. Degrades gracefully to plain nav_to_table
behavior if no native door exists in the loaded room set.
"""
...
def get_task_config() -> TaskConfig:
    return TaskConfig(
        ...
        default_collect_hdf5="demos/behavior1k/teleop_data/nav_door_squeeze.hdf5",
        default_playback_hdf5="demos/behavior1k/playback_data/nav_door_squeeze_playback.hdf5",
        default_video_dir="demos/behavior1k/playback_videos/nav_door_squeeze",
    )
...
def register_teleop_keys(env, kb):
    ...
    try:
        ...
        if cam is None:
            print("[nav_door_squeeze] no robot camera found; skipping head-camera viewport")
            return
        ...
        for _ in range(5):
            og.sim.render()
        print(f"[nav_door_squeeze] head camera '{cam.name}' shown in '{cam._viewport.name}'")
    except Exception as e:
        print(f"[nav_door_squeeze] could not show head-camera viewport: {e}")
