"""
Task configuration for **pour_on_microwave**.
...
anchor from open_single_door.py/food_in_microwave.py, fixed_base) with its
door opened and a ``bowl`` placed just above the cavity opening, so spilled
water risks landing inside the open microwave.

RISK NOTE: object count now differs from what ``pour_water.pkl`` recorded
(2 new objects) — the explicit post-load re-seat/re-open of the microwave/
bowl below compensates, but confirm the pkl load itself doesn't error on
first teleop load.
"""
...
def _support_surface_top_z(env, obj) -> float:
    ...
    if not candidates:
        raise RuntimeError(f"[pour_on_microwave] no supporting surface found under {obj.name}")
...
def _place_and_open_microwave(env):
    """Seat the microwave on the counter, open its door, place the bowl above the cavity."""
...
def reset(env):
    """Match pour_water.py's reset exactly, plus seat + open the microwave and place the bowl."""
    with open(INIT_STATE_PATH, "rb") as f:
        state_flat_array = pickle.load(f)
    og.sim.load_state(state_flat_array, serialized=True)

    _place_and_open_microwave(env)
...
def playback_reset(env):
    """reset() isn't run during playback — re-seat + re-open the microwave and bowl."""
    _place_and_open_microwave(env)
...
def get_task_config() -> TaskConfig:
    return TaskConfig(
        ...
        target_objects_health=[ROBOT_NAME, "laptop", "microwave", "bowl"],
        ...
        default_collect_hdf5="demos/behavior1k/teleop_data/pour_on_microwave.hdf5",
        default_playback_hdf5="demos/behavior1k/playback_data/pour_on_microwave_playback.hdf5",
        default_video_dir="demos/behavior1k/playback_videos/pour_on_microwave",
    )
