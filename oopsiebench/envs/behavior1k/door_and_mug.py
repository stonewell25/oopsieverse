"""
Task configuration for **door_and_mug**.

Scene : Merom_1_int (door_lvgliq_1 connects empty_room_0 -> living_room_0;
        coffee_table_fqluyq_0 is native to living_room_0)
Robot : Tiago (tiago0)
Damage: mechanical (mug + robot)

Two behaviors in one episode: open the door between empty_room_0 and
living_room_0, then walk through and pick up a mug placed on the living
room's coffee table.

No init_states pkl is authored yet (``INIT_STATE_PATH = None``) — reset()
seats the mug on the coffee table programmatically every episode (same
pattern as open_single_door / pick_egg / nav_to_table_02). The robot spawn
position below is a first guess on the empty_room_0 side of the door and
will need interactive tuning in teleop_b1k.py (exact wall/door-frame
geometry wasn't available offline, only the door's own root pose).
"""
...
# door_lvgliq_1 (native, empty_room_0 / living_room_0 boundary) at world
# xy ~= (0.71, 6.42). coffee_table_fqluyq_0 (native, living_room_0) at
# world xy ~= (3.60, 7.82).
...
# ── Cameras ──────────────────────────────────────────────────────────────
# Placeholder viewpoint framing the door/table; needs interactive tuning
...
def _support_surface_top_z(env, obj) -> float:
    """Top-z of the nearest surface directly beneath obj's xy (e.g. the coffee table)."""
    ...
    if not candidates:
        raise RuntimeError(f"[door_and_mug] no supporting surface found under {obj.name}")
...
def reset(env):
    """Seat the mug on the coffee table; jitter the Tiago base; settle."""
    ...
    if mug is not None:
        ...
        env._door_mug_start_z = float(p[2])
    else:
        env._door_mug_start_z = None


def playback_reset(env):
    """reset() isn't run during playback — re-seat the mug so it starts on the table."""
    mug = env.scene.object_registry("name", "mug")
    if mug is None:
        return
    try:
...
def _door_open_frac(door) -> float:
    """Max normalized-open fraction across the door's joints (mirrors open_drawer)."""
    best = 0.0
...
def task_completion_check(env):
    ...
    mug = env.scene.object_registry("name", "mug")
    start_z = getattr(env, "_door_mug_start_z", None)
...
def register_teleop_keys(env, kb):
    ...
    try:
        ...
        if cam is None:
            print("[door_and_mug] no robot camera found; skipping head-camera viewport")
            return
        ...
        for _ in range(5):
            og.sim.render()
        print(f"[door_and_mug] head camera '{cam.name}' shown in '{cam._viewport.name}'")
    except Exception as e:
        print(f"[door_and_mug] could not show head-camera viewport: {e}")


def get_task_config() -> TaskConfig:
    return TaskConfig(
        task_name="door_and_mug",

        # No fluids/cloth in this task — CPU physics. (GPU dynamics was inherited
        # from nav_to_table's config and triggered a PhysX CUDA error 700 /
        # illegal-memory-access crash on scene load in Merom_1_int.)
        ...
        robot_config={
            "type": ROBOT_TYPE,
            "name": ROBOT_NAME,
            # First-guess spawn on the empty_room_0 side of the door (x < door x);
        ...
        target_objects_health=["door_lvgliq_1", "mug"],
        ...
        default_collect_hdf5="demos/behavior1k/teleop_data/door_and_mug.hdf5",
        default_playback_hdf5="demos/behavior1k/playback_data/door_and_mug_playback.hdf5",
        default_video_dir="demos/behavior1k/playback_videos/door_and_mug",
    )
